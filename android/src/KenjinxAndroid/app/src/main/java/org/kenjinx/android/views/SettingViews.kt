...
import org.kenjinx.android.viewmodels.KenjiSettingsImport
...
class SettingViews {
    companion object {
        ...
        fun Main(settingsViewModel: SettingsViewModel, mainViewModel: MainViewModel) {
            val importKenjiSettings = rememberLauncherForActivityResult(ActivityResultContracts.OpenDocument()) { uri ->
                if (uri != null) {
                    val activity = settingsViewModel.activity
                    thread {
                        val result = runCatching { KenjiSettingsImport.importSettings(activity, uri) }
                        activity.runOnUiThread {
                            result.onSuccess { count ->
                                Toast.makeText(activity, "Imported $count Kenji settings. Select your game folder again.", Toast.LENGTH_LONG).show()
                                activity.recreate()
                            }.onFailure { error ->
                ...
                Column(
                    ...
                    ExpandableView(onCardArrowClick = { }, title = "User Interface", icon = Icons.Outlined.BarChart ,isFirst = true) {
                        Column(modifier = Modifier.fillMaxWidth()) {
                            ...
                            ActionButton(
                                onClick = { importKenjiSettings.launch(arrayOf("text/xml", "application/xml", "application/octet-stream")) },
                                text = "Import Kenji-NX Settings (XML)",
                                icon = Icons.Default.FileDownload,
