...
namespace Ryujinx.Common.Utilities
{
    public static class NetworkHelpers
    {
        private static (IPInterfaceProperties, UnicastIPAddressInformation) GetLocalInterface(NetworkInterface adapter, bool isPreferred)
        {
            ...
            // Android does not expose gateway/DNS lists through the desktop .NET APIs.
            // The host resolver and sockets still work; do not reject an IPv4 interface for that.
            if (isPreferred || PlatformInfo.IsBionic || (properties.GatewayAddresses.Count > 0 && properties.DnsAddresses.Count > 0))
            {
        ...
        {
            if (!PlatformInfo.IsBionic && !NetworkInterface.GetIsNetworkAvailable())
            {
                return (null, null);
            }
