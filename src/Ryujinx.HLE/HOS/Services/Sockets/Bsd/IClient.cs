...
#pragma warning restore IDE0059
            ...
            if (socket != null)
            {
                ...
                if (EagleBootstrap.IsBootstrapEndpoint(EagleBootstrap.GetApplicationTitleId(context), endPoint))
                {
                    ...
                    {
                        return WriteBsdResult(context, -1, LinuxError.ENETDOWN);
                    }

                    if (socket is not ManagedSocket managedSocket || socket.SocketType != SocketType.Stream)
                    {
                        return WriteBsdResult(context, -1, LinuxError.EPROTONOSUPPORT);
                    }
                    ...
                    try
                    {
                        IPAddress address = Array.Find(Dns.GetHostAddresses(EagleBootstrap.ServerHost),
                            candidate => candidate.AddressFamily == AddressFamily.InterNetwork);

                        if (address == null)
                        {
                            return WriteBsdResult(context, -1, LinuxError.EHOSTUNREACH);
                        }

                        endPoint = new IPEndPoint(address, EagleBootstrap.ServerPort);
                        managedSocket.RedirectedTlsHostName = EagleBootstrap.ServerHost;
                        Logger.Info?.Print(LogClass.ServiceBsd, $"Eagle bootstrap connect: {EagleBootstrap.ServerHost}:{EagleBootstrap.ServerPort}");
                    }
