...
namespace Ryujinx.HLE.HOS.Services.Nifm.StaticService
{
    class IGeneralService : DisposableIpcService
    {
        ...
        public IGeneralService(ServiceCtx context)
        {
            ...
            ulong applicationTitleId = EagleBootstrap.GetApplicationTitleId(context);
            if (EagleBootstrap.GetBootstrapHost(applicationTitleId) != null)
            {
                Logger.Info?.Print(LogClass.ServiceNifm,
                    $"RetroMods application {applicationTitleId:X16}: internet enabled={context.Device.Configuration.EnableInternetAccess}, requests accepted={_generalServiceDetail.IsAnyInternetRequestAccepted}");
            }
        }
...
#pragma warning restore IDE0059
            ...
            bool accepted = GeneralServiceManager.Get(clientId).IsAnyInternetRequestAccepted;
            context.ResponseData.Write(accepted);
            Logger.Debug?.Print(LogClass.ServiceNifm, $"Internet request accepted={accepted}, client={clientId}");
