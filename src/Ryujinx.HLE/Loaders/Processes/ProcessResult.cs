...
namespace Ryujinx.HLE.Loaders.Processes
{
    public class ProcessResult
    {
        ...
        {
            if (RetroModsNetworkPolicy.RequiresGuestInternet(ProgramId))
            {
                device.Configuration.EnableInternetAccess = true;
                Logger.Info?.Print(LogClass.Loader, $"RetroMods automatically enabled guest internet for {ProgramId:X16}.");
            }
