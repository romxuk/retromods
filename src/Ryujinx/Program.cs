    ...
    internal static class Program
    {
        ...
        public static int Main(string[] args)
        {
            ...
            if (OperatingSystem.IsWindows())
            {
                ...
                // RetroMods permits elevated Windows sessions; log the privilege level and continue.
                if (new WindowsPrincipal(WindowsIdentity.GetCurrent()).IsInRole(WindowsBuiltInRole.Administrator))
                {
                    Logger.Warning?.PrintMsg(LogClass.Application, "Ryujinx is running with administrator privileges. Continuing startup.");
                }
            }
            else // Unix
            {
