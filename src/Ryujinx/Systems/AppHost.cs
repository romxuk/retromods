...
namespace Ryujinx.Ava.Systems
{
    internal class AppHost : IDisposable
    {
        ...
        {
            Device.Configuration.EnableInternetAccess = RetroModsNetworkPolicy.EffectiveGuestInternet(
                e.NewValue, Device.Processes.ActiveApplication?.ProgramId ?? 0);
        }
        ...
        public async Task LoadGuestApplication(CancellationTokenSource cts, BlitStruct<ApplicationControlProperty>? customNacpData = null)
        {
            ...
            if (RetroModsNetworkPolicy.RequiresGuestInternet(Device.Processes.ActiveApplication.ProgramId))
            {
                ConfigurationState.Instance.System.EnableInternetAccess.Value = true;
                Device.Configuration.EnableInternetAccess = true;
            }
